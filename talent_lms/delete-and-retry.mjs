...
async function fix() {
  ...
  // Also delete the auth user just in case to start fresh
  ...
  if (employerError) { console.error(employerError); return; }

  await supabase.auth.admin.updateUserById(userId, {
    user_metadata: { account_type: 'employer', name: 'Admin User', employer_id: employerData.id }
  });

  console.log("Success! email:", email, "password: password123");
}
